...
type ZTToken = { id: string; text: string; locked?: boolean };
...
function splitTokens(input: string, delimiter: string | null): string[] {
  ...
  if (!delimiter || delimiter.trim() === "") {
    // Default: split by whitespace, but if no spaces, split each character
    const tokens = input.trim().split(/\s+/);
    if (tokens.length === 1 && tokens[0].length > 1) {
      // Single word without spaces - split into individual characters
          ...
          <tbody>
            {otRows.map((r, i) => (
              <tr key={`ot-${i}`}>
                ...
                <td className="px-2 py-1">
                  {r.map((c) => (
                    <span key={c.id} className="inline-block min-w-[14px]">
                      {c.ch || "·"}
                    </span>
                  ))}
                </td>
            ...
            {ztRows.map((r, i) => (
              <tr key={`zt-${i}`}>
                ...
                <td className="px-2 py-1">
                  {r.map((t) => (
                    <span key={t.id} className="inline-block min-w-[14px]">
                      {t.text || "·"}
                    </span>
                  ))}
                </td>
